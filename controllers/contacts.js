...
const getContact = async (id) => {
    const results = await getContactsDB().find({ _id: new mongodb.ObjectId(id) }).toArray();
    return results[0];
}
...
const updateContact = async (id, contact) => {
    const results = await getContactsDB().updateOne({ _id: new mongodb.ObjectId(id) }, { $set: contact });
    return contact;
}
